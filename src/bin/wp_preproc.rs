use algae_farmer::wikitext::{normalize_link_target, parse_templates, wp_template_links};
use anyhow::Result;
use clap::Parser;
...
fn extract_wikilinks(re: &Regex, text: &str) -> Vec<String> {
    ...
    for cap in re.captures_iter(text) {
        ...
        // Take part before pipe (link target, not display text)
    ...
    for tmpl in parse_templates(text) {
        for target in wp_template_links(&tmpl) {
            if let Some(clean) = normalize_link_target(target) {
                links.push(clean.to_string());
            }
        }
